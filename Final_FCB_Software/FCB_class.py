...
#             return 1
    ...
    def wait_for_acknowledgment(self, timeout=15):
        """
    ...
    def send_chunk_request(self, buffer, lowerchunk='0'):
        ...
        if lowerchunk.isdigit() and upperchunk.isdigit() and int(lowerchunk) <= int(upperchunk):
            ...
            try:
                ...
                if len(buffer) < len(jpg_bytes):
                    raise ValueError("Buffer is too small for the received data!")
            
                # Copy received bytes into the provided buffer
                buffer[:len(jpg_bytes)] = jpg_bytes
                # Return the portion of the buffer that was filled
                return buffer[:len(jpg_bytes)]
