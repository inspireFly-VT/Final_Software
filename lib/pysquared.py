...
class Satellite:
    ...
    def __init__(self):
        ...
        # --- This needs to be changed before flight
        self.NORMAL_BATT_TEMP= -10#Set to 0 BEFORE FLIGHT!!!!!
...
#         return 0
    ...
    # inspireFly Antenna Function
    def deploy_antenna(self):
        print("TODO: Implement deploy antenna code")
        
        # Send deploy antenna command
        # Test to see if it is actually deployed
        # If not, retry
        
    def deploy_boomarm(self):
        
        # So this will use a mix of checking if the switch has released and if the burn function worked
        
        # Note: if either the done function fails or the switch isnt off, this will retry
        
        # It will only try 5 times so the satellite can still do its operations if there is a major failure
        ...
        if(switch.value == False):
            self.debug_print("ERROR: The switch was deployed before attempting to burn")
            print("This needs to be changed in deploy boomarm!")
            self.burn('1', dutycycle, 1000, 10)
        ...
        for i in range(50):
            ...
            done = self.burn('1', dutycycle, 1000, 1)
            

            
        # This works but backwards. True means its not deployed, false means it is    
            
            
        
        
        # Test to see if it is actually deployed (how?)
        # If not, retry
        # If it continues to fail, maybe increase volage/current?
        # If it keeps failing, break out and return in state of health.
        # Beaconing functionality will still work so just take the L
    ...
    def heater_on(self):
        ...
        self.debug_print("heater_on called")
        
        if self.hardware['FLD']:
           
            self.debug_print("FLD is true")
            ...
            try:
                ...
                self.debug_print("Running heater on")
